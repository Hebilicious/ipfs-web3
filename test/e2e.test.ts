...
describe("IPFSUploadAndRegister E2E Tests", () => {
  ...
  it("should upload a dummy file and register CID", async () => {
    ...
    const result = await web3.IPFSUploadAndRegister.uploadAndStore({
      filePath,
      from: account?.address,
    });
    expect(result.success).toBe(true);
    expect(result.cid.toString()).toBeTypeOf("string");
    expect(result.blockchainState.from.toLowerCase()).toBe(
  ...
  it("should list CIDs for a given address", async () => {
    const cids = await web3.IPFSUploadAndRegister.listCIDs({
      address: account?.address,
    });
    const returnValues = cids.map(
      (c) => typeof c !== "string" && c.returnValues
    );
