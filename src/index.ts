...
import { type AddOptions, unixfs, type UnixFS } from "@helia/unixfs";
import {
  Contract,
  EventLog,
  Web3PluginBase,
...
export class IPFSUploadAndRegister extends Web3PluginBase {
  ...
  public async uploadAndStore({
    ...
    success: boolean;
    cid: CID;
    blockchainState: TransactionReceipt;
  }> {
    if (from && !validator.isAddress(from))
      throw new Error(`Provided address "${from}" is not Ethereum-compatible.`);
    if (!filePath && !file)
      throw new Error("Provide either a filePath or a file.");
    if (filePath && file)
      throw new Error("filePath and file can't be provided simultaneously.");
    if (!isNode && !file)
      throw new Error(
        "File must be provided when running in a browser environment."
      );
  ...
  public async listCIDs({
    ...
    if (!validator.isAddress(address))
      throw new Error(
        `Provided address "${address}" is not Ethereum-compatible.`
      );
    const events = await this.getCIDStore().getPastEvents("CIDStored", {
      fromBlock: 0,
      toBlock: "latest",
   ...
   */
  public async browserUploadFile({
    file,
    options,
  }: {
    file: File;
    ...
    if (!(fileBuffer instanceof Uint8Array))
      throw new Error(
        "Error while reading file. The provided file can't be converted to a Uint8Array."
      );
    const ufs = await this.getFS();
